...
export default async function Page(props: {
  ...
  const course = COURSES.codebaseEntrepreneur.find(c => c.slug === page.slugs[0]);
    ...
    >
      ...
      <DocsBody className="text-fd-foreground/80">
        ...
        <MDX components={{
          ...
          Button, Cards, Card, Callout, Accordion, Accordions, AutoTypeTable, Gallery, Mermaid, Quiz, Flashcard, Popup, PopupContent, PopupTrigger, Step, Steps, Tab, Tabs, TypeTable, YouTube,
...
export async function generateMetadata(props: {
  ...
  const description =
    page.data.description ?? 'Learn entrepreneurship with Codebase Entrepreneur Academy';

  const imageParams = new URLSearchParams();
  imageParams.set('title', `${page.data.title} | Codebase Entrepreneur Academy`);
  imageParams.set('description', description);

  const image = {
    alt: 'Banner',
    url: `/api/og/codebase-entrepreneur-academy/${params.slug[0]}?${imageParams.toString()}`,
    width: 1200,
    height: 630,
  };
