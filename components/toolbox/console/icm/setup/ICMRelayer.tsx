...
function ICMRelayerInner({ onSuccess }: BaseConsoleToolProps) {
    ...
    const currentStep = useMemo(() => {
        if (!error && selectedSources.length > 0 && selectedDestinations.length > 0 && configJson) return 4; // Ready to run
        if (!error && selectedSources.length > 0 && selectedDestinations.length > 0) return 3; // On save config
        if (relayerAddress) return 2; // On configure
        return 1; // On docker installation
    }, [relayerAddress, selectedSources, selectedDestinations, error, configJson]);

    const completedSteps = useMemo(() => {
        const completed: number[] = [];
        completed.push(1); // Docker installation (informational)
        if (selectedSources.length > 0 && selectedDestinations.length > 0 && !error) {
            completed.push(2); // Configuration complete
            if (configJson) completed.push(3); // Config preview available
        }
        // Step 4 is informational (run command), doesn't need completion tracking
        return completed;
    }, [selectedSources, selectedDestinations, error, configJson]);
