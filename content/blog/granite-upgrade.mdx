...
description: The Avalanche Granite upgrade activates on Mainnet on [DATE_TBD], bringing major improvements including P-Chain epoched views for ICM, secp256r1 support for biometric authentication, and dynamic minimum block times for faster transactions.
date: 2025-10-18
...
The latest upgrade to the Avalanche Network, dubbed "Avalanche Granite", is scheduled to activate on the **Mainnet on [DATE_TBD]**. This upgrade builds upon the successful activation and testing on the Avalanche Fuji Testnet, which took place on [DATE_TBD].
...
Fuji node operators must upgrade their nodes to **v1.xx-fuji [TBD]** prior to the activation time on [DATE_TBD]. The pre-release includes protocol optimizations that are not compatible with AvalancheGo versions < v1.xx.x-fuji [TBD]. The plugin version is unchanged at 39 and is compatible with version v1.xx.x [TBD].
